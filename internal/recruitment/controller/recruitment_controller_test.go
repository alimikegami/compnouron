...
func TestCreateRecruitmentApplicationForeignKeyError(t *testing.T) {
	mockUseCase := mocks.NewRecruitmentUseCase(t)
	mockUseCase.On("CreateRecruitmentApplication", dto.RecruitmentApplicationRequest{
		RecruitmentID: 999,
	}, uint(1)).Return(errors.New("Error 1452: Cannot add or update a child row: a foreign key constraint fails (`compnouron`.`recruitment_applications`, CONSTRAINT `fk_recruitment_applications_recruitment` FOREIGN KEY (`recruitment_id`) REFERENCES `recruitments` (`id`))"))
